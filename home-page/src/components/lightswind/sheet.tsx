	...
	({ children, asChild, ...props }, forwardedRef) => {
		...
		if (asChild) {
			if (!React.isValidElement(child)) {
				console.error(
					'SheetTrigger with `asChild` expects a single valid React element child.'
				)
				return null
			}
	...
	({ side = 'right', className, children, ...props }, ref) => {
		...
		React.useEffect(() => {
			...
			const handleMouseDown = (event: MouseEvent) => {
				if (
					contentLocalRef.current &&
					!contentLocalRef.current.contains(event.target as Node)
				) {
					setOpen(false)
				}
			}
					...
					<SheetPortal>
						...
						<motion.div
							...
							className={cn(
								...
								side === 'top' &&
									'inset-x-0 top-0 border-b border-gray-200 dark:border-gray-800/40',
								side === 'bottom' &&
									'inset-x-0 bottom-0 border-t border-gray-200 dark:border-gray-800/40',
								side === 'left' &&
									'inset-y-0 left-0 h-full w-3/4 border-r border-gray-200 sm:max-w-sm dark:border-gray-800/40',
								side === 'right' &&
									'inset-y-0 right-0 h-full w-3/4 border-l border-gray-200 sm:max-w-sm dark:border-gray-800/40',
								className
							)}
							{...restProps}
						>
