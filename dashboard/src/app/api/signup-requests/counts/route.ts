...
export async function GET(
	...
	try {
		const signupRequestsCount = await signupService.findPendingRequests()

		return apiResponse(
			{
				success: true,
				message: 'signup Request Counts returned successfully from database',
				data: signupRequestsCount,
			},
			{ status: 200 }
		)
	} catch (error) {
		if (error instanceof NotFoundError)
			return NextResponse.json(
				{ success: false, message: 'singup Requests Counts not found', data: [] },
				{ status: 404 }
			)
