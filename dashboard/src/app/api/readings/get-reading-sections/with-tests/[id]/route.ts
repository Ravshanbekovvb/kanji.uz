...
export async function GET(
	...
	try {
		const readingSectionWithTests = await readingsService.findReadingSectionWithTestsById(id)

		return apiResponse({
			success: true,
			message: 'Reading Section with tests returned successfully!',
			data: readingSectionWithTests,
		})
	} catch (error) {
